...
interface NpcModalProps {
  readonly initial: SceneNpc | null;
  readonly existingIds: readonly string[];
  readonly onSave: (npc: SceneNpc) => void;
  readonly onClose: () => void;
}

export function NpcModal({ initial, existingIds, onSave, onClose }: NpcModalProps): JSX.Element {
  ...
  const handleSave = useCallback(() => {
    ...
    const othersIds = initial !== null ? existingIds.filter((id) => id !== initial.id) : existingIds;
  ...
  return (
    <ModalBackdrop onClose={onClose} wide>
      <h2 style={{ margin: "0 0 16px", fontSize: "1.1rem", color: "#e0e0e0" }}>
        {initial !== null ? "Editar NPC" : "Nuevo NPC"}
      </h2>
